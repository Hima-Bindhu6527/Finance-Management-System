...
const GoalsList = ({ goals, onGoalsChange, loading }) => {
  ...
  const fetchHistory = async (goalId, goalName) => {
    ...
    try {
      ...
      const res = await fetch(`http://localhost:5000/api/goals/${goalId}/history`, {
        headers: {
      ...
      if (res.ok && data.data) {
        ...
        const progressHistory = Array.isArray(data.data.progressHistory)
          ...
          : [];
        progressHistory.sort((a, b) => new Date(b.timestamp) - new Date(a.timestamp));
                ...
                <button
                  ...
                  onClick={async () => {
                    ...
                    const val = window.prompt(
                      `Current Amount: ${formatCurrency(goal.currentAmount || 0)}\n\nEnter amount to add:`,
                      '0'
                    );
                    if (val === null) return;
                    ...
                    try {
                      ...
                      if (res.ok) {
                        ...
                        alert(
                          `Progress updated!\n\n` +
                          `Previous: ${formatCurrency(goal.currentAmount || 0)}\n` +
        ...
        >
          ...
          >
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.5rem' }}>
              ...
              <button onClick={() => setShowHistory(false)} style={{ padding: '0.25rem 0.5rem' }}>Close</button>
            </div>
            ...
            {historyLoading ? (
              ...
              <div style={{ padding: '1rem', textAlign: 'center', color: '#666' }}>
                ...
                <p style={{ fontSize: '0.9rem' }}>Updates will appear here when you add contributions.</p>
              </div>
            ) : (
              <div style={{ maxHeight: '60vh', overflowY: 'auto' }}>
                <div style={{ marginBottom: '1rem', padding: '0.75rem', background: '#f3f4f6', borderRadius: '4px' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '0.5rem', marginBottom: '0.5rem' }}>
                      ...
                      <div>
                        ...
                        <div style={{
                          ...
                          {entry.amount >= 0 ? '+' : ''}{formatCurrency(entry.amount || 0)}
                        </div>
                      </div>
                    ...
                    {entry.action && (
                      <div style={{ marginTop: '0.5rem' }}>
                        <span style={{
                          ...
                          background: entry.action === 'contribution' ? '#dbeafe' : '#fef3c7',
                          color: entry.action === 'contribution' ? '#1e40af' : '#92400e',
                          borderRadius: '12px',
